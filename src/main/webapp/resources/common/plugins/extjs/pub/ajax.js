...
function sendRequest(url,divId) {
	...
	var oldmsg =  window.document.getElementById(divId).innerHTML;
	window.document.getElementById(divId).innerHTML="<img src='/manage/images/loading.gif'/>";
    	...
    	if (XMLHttpReq.readyState == 4) { // 判断对象状态
	        if (XMLHttpReq.status == 200) { // 信息已经成功返回，开始处理信息	 
	            ...
	            if(doc.indexOf('error')>=0){
	           		alert("出错，操作失败。");
	           		window.document.getElementById(divId).innerHTML=oldmsg;
	            }else if(doc.indexOf('null')>=0){
	            	alert("操作失败。");
	           		window.document.getElementById(divId).innerHTML=oldmsg;
	            }else{
	        		window.document.getElementById(divId).innerHTML=doc;
	        	}
	        } else { //页面不正常
	           alert("操作失败,发生错误。");
	           window.document.getElementById(divId).innerHTML=oldmsg;
