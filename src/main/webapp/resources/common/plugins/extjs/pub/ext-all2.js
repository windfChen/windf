		...
		.extend(
				Ext.Component,
				{
					...
					update : function(W) {
						...
						F += C;
						var P = 86400000;
						var U = (new Date(S.getFullYear(), S.getMonth(), D))
								.clearTime();
						var T = new Date().clearTime().getTime();
						var N = W.clearTime().getTime();
						var M = this.minDate ? this.minDate.clearTime()
								: Number.NEGATIVE_INFINITY;
						var Q = this.maxDate ? this.maxDate.clearTime()
								: Number.POSITIVE_INFINITY;
						var X = this.disabledDatesRE;
						var L = this.disabledDatesText;
						var Z = this.disabledDays ? this.disabledDays.join("")
								: false;
						...
						this.mbtn.setText(this.monthNames[W.getMonth()] + " "
								+ W.getFullYear());
		...
		.extend(
				Ext.Panel,
				{
					...
					onRender : function(C, A) {
						...
						if (this.plain) {
							var E = this.tabPosition == "top" ? "header"
									: "footer";
				...
				Ext.Component,
				{
					...
					monitorMouseOver : function(A) {
						if (A.target != this.el.dom && !A.within(this.el)) {
							Ext.getDoc().un("mouseover", this.monitorMouseOver,
									this);
							this.onMouseOut(A)
						}
					},
		...
		.extend(Ext.Editor, Ext.Component,
				{
					...
					initComponent : function() {
						...
						this.addEvents("beforestartedit", "startedit",
								"beforecomplete", "complete", "specialkey")
					},
					onRender : function(B, A) {
						this.el = new Ext.Layer( {
							shadow : this.shadow,
		...
		.extend(
				...
				Ext.data.Node,
				{
					...
					setFirstChild : function(A) {
						...
						Ext.tree.TreeNode.superclass.setFirstChild
								.call(this, A);
					...
					appendChild : function() {
						var A = Ext.tree.TreeNode.superclass.appendChild.apply(
								this, arguments);
					...
					removeChild : function(A) {
						...
						Ext.tree.TreeNode.superclass.removeChild.apply(this,
								arguments);
					...
					isSelected : function() {
						return this.getOwnerTree().getSelectionModel()
								.isSelected(this)
					},
					...
					collapse : function(B, E) {
						if (this.expanded && !this.isHiddenRoot()) {
							...
							this.expanded = false;
							if ((this.getOwnerTree().animate && E !== false)
									|| E) {
					...
					ensureVisible : function(B) {
						...
						A.expandPath(this.parentNode.getPath(), false,
								function() {
									...
									A.getTreeEl().scrollChildIntoView(
											C.ui.anchor);
									Ext.callback(B)
								}.createDelegate(this))
					},
					...
					sort : function(E, D) {
						Ext.tree.TreeNode.superclass.sort
								.apply(this, arguments);
						if (this.childrenRendered) {
							var C = this.childNodes;
							for ( var B = 0, A = C.length; B < A; B++) {
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderElements : function(D, I, H, J) {
		...
		var C = [
				...
				(I.iconCls ? " " + I.iconCls : ""),
				"\" unselectable=\"on\" />",
				E ? ("<input class=\"x-tree-node-cb\" type=\"checkbox\" " + (I.checked ? "checked=\"checked\" />"
						: "/>"))
						: "",
...
Ext.menu.MenuMgr = function() {
	...
	function J(N) {
		...
		if (N.parentMenu) {
			N.getEl().setZIndex(
					parseInt(N.parentMenu.getEl().getStyle("z-index"), 10) + 3);
			N.parentMenu.activeChild = N
		} else {
			if (O && O.isVisible()) {
				N.getEl().setZIndex(
						parseInt(O.getEl().getStyle("z-index"), 10) + 3)
			}
		}
	}
	function B(N) {
		...
		.extend(
				Ext.form.TriggerField,
				{
					...
					restrictHeight : function() {
						...
						var F = Ext.lib.Dom.getViewHeight() - A
								- this.getSize().height;
						...
						for ( var E = 0, A = B.length; E < A; E++) {
							...
							D
									.push(
											...
											C,
											";\"",
											(this.defaultFont == F ? " selected=\"true\">"
													: ">"), C, "</option>")
						}
						return D.join("")
					},
				...
				Ext.util.Observable,
				{
					...
					doRender : function(E, G, M, A, L, Q) {
						...
						for ( var P = 0, S = G.length; P < S; P++) {
							...
							for ( var R = 0; R < L; R++) {
								U = E[R];
								O.id = U.id;
								O.css = R == 0 ? "x-grid3-cell-first "
										: (R == H ? "x-grid3-cell-last " : "");
								O.attr = O.cellAttr = "";
								O.value = U.renderer(K.data[U.name], O, K, J,
										R, M);
								O.style = U.style;
								...
								if (K.dirty
										&& typeof K.modified[U.name] !== "undefined") {
					...
					renderUI : function() {
						...
						if (C.enableColumnResize !== false) {
							this.splitone = new Ext.grid.GridView.SplitDragZone(
									C, this.mainHd.dom)
						}
					...
					ensureVisible : function(P, E, D) {
						...
						return F ? Ext.fly(F).getXY() : [ L.scrollLeft,
								Ext.fly(I).getY() ]
					},
					...
					fitColumns : function(D, G, E) {
						...
						while (I.length) {
							H = I.pop();
							O = I.pop();
							F.setColumnWidth(O, Math.max(
									this.grid.minColumnWidth, Math.floor(H + H
											* C)), true)
						}
						if ((R = F.getTotalWidth(false)) > J) {
							var Q = P != A ? E : N;
							F.setColumnWidth(Q, Math.max(1, F.getColumnWidth(Q)
									- (R - J)), true)
						}
						if (D !== true) {
					...
					autoExpand : function(B) {
						...
						if (!this.userResized && G.autoExpandColumn) {
							...
							var H = this.grid.getGridEl().getWidth(true)
									- this.scrollOffset;
							if (D != H) {
								var F = A.getIndexById(G.autoExpandColumn);
								var E = A.getColumnWidth(F);
								var C = Math.min(Math.max(((H - D) + E),
					...
					onRowOut : function(B, A) {
						...
						if ((C = this.findRowIndex(A)) !== false
								&& C !== this
										.findRowIndex(B.getRelatedTarget())) {
				...
				Ext.grid.GridView,
				{
					...
					doRender : function(D, G, P, A, O, R) {
						...
						for (T = 0, U = G.length; T < U; T++) {
							var J = A + T;
							var L = G[T], E = L.data[Y], V = this.getGroup(E,
									L, B, J, N, P);
							if (!K || K.group != V) {
								M = Q + "-gp-" + Y + "-"
						...
						for (T = 0, U = X.length; T < U; T++) {
							...
							W[W.length] = Ext.grid.GroupingView.superclass.doRender
									.call(this, D, V.rs, P, V.startRow, O, R);
...
Ext.extend(Ext.grid.HeaderDropZone, Ext.dd.DropZone, {
	...
	onNodeDrop : function(B, K, F, D) {
		...
		if (E != B) {
			...
			var A = Ext.lib.Dom.getRegion(B.firstChild);
			var L = (A.right - H) <= ((A.right - A.left) / 2) ? "after"
					: "before";
				...
				Ext.BoxComponent,
				{
					...
					onRender : function(D, A) {
						...
						if (this.textEl) {
							...
							this.textTopEl = Ext
									.get(this.progressBar.dom.firstChild);
							var E = Ext.get(B.childNodes[1]);
							this.textTopEl.setStyle("z-index", 99).addClass(
									"x-hidden");
					...
					wait : function(B) {
						if (!this.waitTimer) {
							var A = this;
							B = B || {};
							this.waitTimer = Ext.TaskMgr
									.start( {
										run : function(C) {
											var D = B.increment || 10;
											this
													.updateProgress(((((C + D) % D) + 1) * (100 / D)) * 0.01)
										},
				...
				Ext.BoxComponent,
				{
					...
					initComponent : function() {
						...
						this.keyIncrement = Math.max(this.increment,
								this.keyIncrement);
						this.addEvents("beforechange", "change", "dragstart",
								"drag", "dragend");
						if (this.vertical) {
					...
					onDrag : function(A) {
						var B = this.innerEl.translatePoints(this.tracker
								.getXY());
						this.setValue(Math.round(B.left / this.getRatio()),
								false);
		...
		.extend(
				Ext.Toolbar,
				{
					...
					clearStatus : function(C) {
						...
						var B = C.useDefaults ? this.defaultText : "", A = C.useDefaults ? this.defaultIconCls
								: "";
						if (C.anim) {
...
Ext.grid.GridView.prototype.templates.cell = new Ext.Template(
		...
		'</td>');
//ʷ���� 2010��12��27�� Exjts2.2.1��datefield�ؼ���IE8����ʾ��ȫBUG���  
